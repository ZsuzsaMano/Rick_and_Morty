...
const clearTable = () => {
  ...
  for (i = 0; i < tableRows; i++) table.deleteRow(1);
};
...
const isCharacter = (item) => {
  //get value from form
  let characterValues = [];
  const checkedCharacters = document.querySelectorAll(
  ...
  return (characterValues.includes('Morty') && item.name.includes('Morty')) ||
    ...
    (characterValues.includes('Other') &&
      item.name.indexOf('Morty') === -1 &&
      item.name.indexOf('Rick') === -1)
    ? true
    : false;
};
...
const isSpecies = (item) => {
  //get value from form
  const speciesValue = document.querySelector('input[name="species"]:checked')
    .value;
...
const fillTable = (arr) => {
  arr.forEach((x) => {
    if (isEpisode(x) && isGender(x) && isSpecies(x) && isCharacter(x)) {
      ...
      col5.innerHTML = x.episode
        .map((episode) => episode.substring(40))
        .join(', ');
    }
  });
};
